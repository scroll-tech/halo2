...
fn rand_2d_array<F: PrimeField, R: RngCore, const W: usize, const H: usize>(
    rng: &mut R,
) -> [[F; H]; W] {
    [(); W].map(|_| [(); H].map(|_| F::random(&mut *rng)))
}

fn shuffled<F: PrimeField, R: RngCore, const W: usize, const H: usize>(
    original: [[F; H]; W],
    rng: &mut R,
) -> [[F; H]; W] {
...
struct MyCircuit<F: PrimeField, const W: usize, const H: usize> {
    original: Value<[[F; H]; W]>,
    shuffled: Value<[[F; H]; W]>,
}

impl<F: PrimeField, const W: usize, const H: usize> MyCircuit<F, W, H> {
...
impl<F: PrimeField, const W: usize, const H: usize> Circuit<F> for MyCircuit<F, W, H> {
