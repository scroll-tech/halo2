...
trait FieldInstructions<F: PrimeField>: AddInstructions<F> + MulInstructions<F> {
...
trait AddInstructions<F: PrimeField>: Chip<F> {
    /// Variable representing a number.
    type Num;
...
trait MulInstructions<F: PrimeField>: Chip<F> {
    /// Variable representing a number.
    type Num;
