...
#[derive(Clone, Debug)]
pub struct EvaluationDomain<F: PrimeField + WithSmallOrderMulGroup<3>> {
    n: u64,
    k: u32,
    extended_k: u32,
    omega: F,
...
impl<F: PrimeField + WithSmallOrderMulGroup<3>> EvaluationDomain<F> {
    ...
    pub fn divide_by_vanishing_poly(
        ...
        parallelize(&mut a.values, |h, mut index| {
            for h in h {
                h.mul_assign(&self.t_evaluations[index % self.t_evaluations.len()]);
                index += 1;
            }
        });
...
#[allow(dead_code)]
#[derive(Debug)]
pub struct PinnedEvaluationDomain<'a, F: PrimeField> {
    k: &'a u32,
    extended_k: &'a u32,
    omega: &'a F,
}
