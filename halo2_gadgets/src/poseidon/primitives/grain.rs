...
pub(super) struct Grain<F: PrimeField> {
    state: BitArr!(for 80, in u8, Msb0),
    next_bit: usize,
    _field: PhantomData<F>,
}

impl<F: PrimeField + FromUniformBytes<64>> Grain<F> {
    ...
    pub(super) fn next_field_element_without_rejection(&mut self) -> F {
        ...
        // calling F::from_uniform_bytes. PLEASE DO NOT COPY THIS INTO YOUR OWN CODE!
        let view = bytes.as_mut();
        for (i, bit) in self.take(F::NUM_BITS as usize).enumerate() {
            // If we diverged from the reference impl and interpreted the bits in LSB
            // order, we would remove this line.
...
impl<F: PrimeField + FromUniformBytes<64>> Iterator for Grain<F> {
    type Item = bool;
...
#[cfg(test)]
mod tests {
    use halo2_proofs::curves::pasta::Fp;
